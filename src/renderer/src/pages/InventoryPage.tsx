...
function ProductForm({ product, onClose, onSave }: ProductFormProps): JSX.Element {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (product) {
        const r = await window.electronAPI.inventory.updateProduct(product.id, payload)
        if (r.success) {
          showToast('success', 'Producto actualizado')
          onSave()
        } else showToast('error', r.error || 'Error al actualizar el producto')
      } else {
        const r = await window.electronAPI.inventory.createProduct(payload)
        if (r.success) {
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                <select
                  ...
                  value={form.category}
                  onChange={(e) =>
                    setForm((p) => ({ ...p, category: e.target.value as Product['category'] }))
                  }
                >
...
function MovementForm({ product, onClose, onSave }: MovementFormProps): JSX.Element {
  ...
  const [price, setPrice] = useState(String(product.cost || product.price || 0))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const priceNum = type === 'in' ? 0 : Number(price) || 0
    if (qty <= 0) {
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="modal-body">
            ...
            {type === 'in' && (
              ...
              >
                La entrada solo incrementa el stock. No genera valor de venta.
              </p>
            )}
            <div className="form-group">
              <label className="label-md">Descripción</label>
...
export function InventoryPage(): JSX.Element {
  ...
  const loadMovements = async () => {
    const r = await window.electronAPI.inventory.getMovements(undefined, {
      page: movementsPage,
      pageSize,
    })
    if (r.success && r.data) {
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: 'flex', gap: 0, marginBottom: 24 }}>
        ...
        <button
          className={`btn ${tab === 'movements' ? 'btn-primary' : 'btn-secondary'}`}
        ...
        <>
          ...
          >
            ...
            <div className="metric-card" style={{ borderColor: 'rgba(255, 180, 171, 0.3)' }}>
              ...
              />
              <div className="metric-card-label" style={{ color: 'var(--color-error)' }}>
                Stock Bajo
              </div>
          ...
          {products.length === 0 ? (
            <div
              ...
              style={{ textAlign: 'center', padding: 48, borderRadius: 12 }}
            >
              ...
              <p
                className="label-md"
                style={{ color: 'var(--color-on-surface-variant)', marginTop: 8 }}
              >
                Ajuste su búsqueda o filtros
              </p>
            </div>
          ) : (
            <>
                    ...
                    >
                      ...
                      >
                        <span
                          className="label-md"
                          style={{
                            background: categoryColors[p.category] + '22',
                        ...
                        <span
                          className={`status-badge status-badge-${level}`}
                          style={{
                            borderRadius: 9999,
                            padding: '2px 8px',
                            fontSize: '10px',
                      ...
                      <div style={{ flex: 1 }}>
                        <h3
                          ...
                          style={{ fontSize: 'clamp(16px, 2vw, 20px)', margin: '0 0 4px 0' }}
                        >
                          {p.name}
                        </h3>
                        <div
                          className="label-xl"
                          style={{ color: 'var(--color-primary-container)', marginBottom: 8 }}
                        >
                      ...
                      <div>
                        ...
                        >
                          ...
                          <span className="label-md" style={{ color }}>
                            {p.stock} unds
                          </span>
                        </div>
              ...
              <tbody>
                {movements.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>
                      <span
                        className={`status-badge status-badge-${m.type === 'in' ? 'success' : 'error'}`}
                        style={{
                          borderRadius: 9999,
                          padding: '2px 8px',
                          fontSize: '10px',
                    ...
                    >
                      {m.type === 'in' ? '—' : formatCurrency(m.price)}
                    </td>
                    <td
                      style={
                        m.type === 'in'
                    ...
                    >
                      {m.type === 'in' ? '—' : formatCurrency(m.total)}
                    </td>
